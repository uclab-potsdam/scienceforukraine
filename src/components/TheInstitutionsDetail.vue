  ...
  <div class="the-detail">
    <header class="max-width">
      <div class="max-width-inner">
        <router-link :to="{ name: 'institutions' }">← all submissions</router-link>
        <span v-if="prev && next">
          <router-link :to="{ name: 'institutionsDetail', params: {id: prev} }">← </router-link>
          <span class="dimmed">{{index + 1}}/{{items.length}}</span>
          <router-link :to="{ name: 'institutionsDetail', params: {id: next} }">  →</router-link>
        </span>
      </div>
    </header>
      ...
      <div class="max-width-inner">
        ...
        <div class="fields">
          ...
          <template v-if="item.about">
            ...
            <p v-for="(paragraph, i) in item.about?.split('\\n').filter(p => p.trim() !== '')" :key="i">{{ paragraph }}</p>
          </template>
